...
class MY_MoCo_V90(BaseModel):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(MY_MoCo_V90, self).__init__(init_cfg)
        self.encoder_q = nn.Sequential(
            build_backbone(backbone), build_neck(neck_gl),
            build_neck(neck_loc)
        )
        ...
        self.neck_q_gl = self.encoder_q[1]
        self.neck_k_gl = self.encoder_k[1]
        self.neck_q_loc = self.encoder_q[2]
        self.neck_k_loc = self.encoder_k[2]
    ...
    def forward_test(self, img, **kwargs):
        ...
        im_loc, order = random_choice(im_loc_tensor)  # 这里的操作是让 batch_size个图像都经过同样的裁剪
        img4 = img[3]  ## image inpainting
        batch_size = im_q.size(0)   ### 经过数据增强后，每个分支的图像的 var都是不同的

        q_features = [self.backbone(im_q)[-1]]  # index-0 means the stage 3
        q_global = q_features[0]  ### for global  contanstive
        q_global = self.neck_q_gl([q_global])[0]  # queries: NxC

        q_gl_var = q_global.var()
        print('after neck q_gl_var',q_gl_var)

        q_global = nn.functional.normalize(q_global, dim=1)  # normalize 默认是l2 范数

        img4_features = [self.backbone(img4)[-1]]  # index-0 means the stage 3
        img4_global = img4_features[0]  ### for global  contanstive
        img4_global = self.neck_q_gl([img4_global])[0]  # queries: NxC
        img4_global = nn.functional.normalize(img4_global, dim=1)  # normalize

        src_features = [self.backbone(im_loc_tensor)[-1]]  # index-0 means the stage 3
        src_global = src_features[0]  ### for global  contanstive
        src_global = self.neck_q_gl([src_global])[0]  # queries: NxC
        src_global = nn.functional.normalize(src_global, dim=1)  # normalize

        # compute key features
        with torch.no_grad():  # no gradient to keys

            k_global = [self.backbone_k(im_k)[-1]][0]  # index-0 means the stage 3
            k_loc = [self.backbone_k(im_loc)[-1]][0]  # index-0 means the stage 3
            ### normalize
            k_global = self.neck_k_gl([k_global])[0]  # queries: NxC
            k_global = nn.functional.normalize(k_global, dim=1)  # normalize

            k_loc = self.neck_k_loc([k_loc])[0]  # queries: NxC
            k_loc_var = k_loc.var()
            print('after neck k_loc_var', k_loc_var)
            k_loc = nn.functional.normalize(k_loc, dim=1)  # normalize

            print('im_loc type is', type(k_loc))
            print('im_loc size is', k_loc.size())

        ## 开始计算 四个分支的 encoding 的方差
        q_gl_var = torch.var(q_global)
        k_gl_var = torch.var(k_global)
        k_loc_var = torch.var(k_loc)
        img4_gl_var = torch.var(img4_global)
        src_gl_var = torch.var(src_global)

        print("q_gl_var",q_gl_var)
        print("k_gl_var",k_gl_var)
        print("k_loc_var", k_loc_var)
        print("img4_gl_var",img4_gl_var)
        print("src_gl_var", src_gl_var)

        hh

        return q_gl_var,k_gl_var,k_loc_var,img4_gl_var
