...
from .align_featuremap import align_featuremap_dist, random_choice, UnNormalize


@ALGORITHMS.register_module()
class MY_MoCo_V95(BaseModel):
    """MoCo.
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(MY_MoCo_V95, self).__init__(init_cfg)
        ...
        self.register_buffer("queue_loc", torch.randn(feat_dim,
                                                      queue_len))  ## create a constaant of (128,16636) it's value is random
    ...
    def forward_train(self, img, order, **kwargs):
        ...
        im_q = img[1]  ## montage stronger aug
        im_k = img[3]  ## 这里是 view生成中指定的新的 aug weakly-aug

        im_loc_q = img[2]  ## random chose stronger aug
        im_loc_k = img[0] ### weakly aug 这里是指由 dataset 指定的aug weakly aug

        batch_size = im_q.size(0)
        # print('order is',order)
        img4 = img[4]  ## image inpainting
        order_list = order
        ...
        q_loc_features = [self.backbone(im_loc_q)[-1]]  # index-0 means the stage 3
        q_loc = q_loc_features ### for global  contanstive
        q_1, q_2, q_3, q_4 = align_featuremap_dist(q_loc, order_list)
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            im_loc_k, idx_unshuffle_loc = batch_shuffle_ddp(im_loc_k)

            k_global = [self.backbone_k(im_k)[-1]][0]  # index-0 means the stage 3
            k_loc = [self.backbone_k(im_loc_k)[-1]]  # index-0 means the stage 3
        ...
        index_k_grid = torch.gather(k_loc, 2,
                                    dense_sim_ind.unsqueeze(1).expand(-1, k_loc.size(1), -1))  ### obtain ites value
        ...
        ### json log
        filename = 'l_mean_gl_v5.json'
        data = {'l_pos_mean': str(l_pos_gl_mean), 'l_neg_mean': str(l_neg_gl_mean), 'l_neg_var': str(l_neg_gl_var),
                'l_mix_gl': str(l_mix_gl),
                'l_pos_loc_mean': str(l_pos_loc_mean), 'l_neg_loc_mean': str(l_neg_loc_mean),
                'l_neg_loc_var': str(l_neg_loc_var),
