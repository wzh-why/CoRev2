'''该脚本对应于 在局部分支中，独立创建一个视图进行局部分支对比'''
...
class MY_MoCo_V11(BaseModel):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(MY_MoCo_V11, self).__init__(init_cfg)
    ...
    def forward_train(self, img, order, **kwargs):
        ...
        im_q = img[0]  ## src transfer
        im_k = img[1]  ## montage
        im_loc_q = img[2]  ### 新增的view
        im_loc_k = img[3]  ## 随机组合的试图
        ...
        ### 新建一个独立的view3
        loc_q_features = [self.backbone(im_loc_q)[-1]]  # index-0 means the stage 3
        q_1 = loc_q_features[0][:, :, :2, :2]
        q_2 = loc_q_features[0][:, :, :2, 2:7]
        q_3 = loc_q_features[0][:, :, 2:7, :2]
        q_4 = loc_q_features[0][:, :, 2:7, 2:7]
        ...
        ## reconstruction loss
        rec_loss = nn.L1Loss()
        src_area = im_q[:, :, 80:144, 80:144]  ### 水平翻转后，该视图可能对应的位置是错误的
