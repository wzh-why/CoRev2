...
''' It is multi-level contrastive of v6 '''

@ALGORITHMS.register_module()
class MY_MOCO_V64(BaseModel):
    """MoCo.
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(MY_MOCO_V64, self).__init__(init_cfg)
    ...
    def forward_train(self, img, mask, **kwargs):
        ...
        masks = torch.unsqueeze(mask,dim=1)  ## obtain the mask
        batch_num = im_q.size()[0]
        ...
        src_area = masks*im_q  ## [16,1,224,224] *dot product im_q [16,3,224,224]
        re_area  = masks*re_img  ## only need cutouted area
        losses['loss_reconstruction'] = rec_loss(src_area, re_area)/(batch_num*3)
        self._dequeue_and_enqueue(k)

        ### json log
        filename = 'l_mean.json'
        data = {'l_pos_mean':str(l_pos_mean),'l_neg_mean':str(l_neg_mean),'l_neg_var':str(l_neg_var),'l_mix':str(l_mix)}
