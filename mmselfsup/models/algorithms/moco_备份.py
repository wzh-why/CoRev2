...
class MoCo(BaseModel):
    ...
    def extract_feat(self, img):
        ...
        x_tuple = tuple([x])
        ### 做了一个修改，只取最后一层的卷积层，随后送入到neck中
        x_neck = self.neck(x_tuple)[0]
        x_norm = nn.functional.normalize(x_neck, dim=1)
    ...
    def forward_test(self, img, **kwargs):
        ...
        im_q = img[0]
        im_k = img[1]
        im_q_var = float(im_q.var())
        im_k_var = float(im_k.var())
        ...
        # im_q_var = float(im_q.var())
        # im_k_var = float(im_k.var())
        # im_loc_var = float(im_loc.var())
        # img4_var = float(img4.var())
        #
        # return im_q_var,im_k_var,im_loc_var,img4_var
        ...
        # q_var = float(q.var())
        #
        # img4_f = self.encoder_q(img4)[0]
        # img4_f_var = float(img4_f.var())
        #
        # # compute key features
        # with torch.no_grad():  # no gradient to keys
        #
        ...
        #     k_var = float(k.var())
        #
        #     k_loc = self.encoder_k(im_loc)[0]
        #     k_loc_var = float(k_loc.var())

        # return q_var,k_var,k_loc_var,img4_f_var
