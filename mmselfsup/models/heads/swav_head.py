...
class SwAVHead(BaseModule):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            self.prototypes.weight.copy_(w)  ### 直接让 prototypes 实现为一个 fc层，并且在这里对其权重进行归一化后，覆盖原来的权重 todo：值得借鉴

        embedding, output = x, self.prototypes(x)  ### 这里的embedding 是zi，size为 全部的特征向量 ，out 是全部的特征与 3000个聚类中心点乘后的结果
        embedding = embedding.detach()

        bs = int(embedding.size(0) / sum(self.num_crops))
        loss = 0
        for i, crop_id in enumerate(self.crops_for_assign): ### 只取其视图和增强视图的特征
            with torch.no_grad():
                ...
                # get assignments (batch_size * num_prototypes) 输入 out(向量与聚类中心的乘积后，再输入sinkhorn算法获得分配？？ ) todo: 还需要理解
                q = distributed_sinkhorn(out, self.sinkhorn_iterations,
                                         self.world_size, self.epsilon)[-bs:]

            # cluster assignment prediction
            subloss = 0
            for v in np.delete(np.arange(np.sum(self.num_crops)), crop_id): ###删除第0、1行
                x = output[bs * v:bs * (v + 1)] / self.temperature ###目的是 为了取除了当前索引外的其他 x的特征 与 聚类中心的乘积 进行运算
                subloss -= torch.mean(
