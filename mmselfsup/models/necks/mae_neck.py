...
class MAEPretrainDecoder(BaseModule):
    ...
    def forward(self, x, ids_restore):
        # embed tokens
        x = self.decoder_embed(x) ### 先经过一个linear map，先将特征维度从768——》512

        # append mask tokens to sequence
        ## 根据之前保存的ids_restore 将mask的张量添加到完整序列中
        mask_tokens = self.mask_token.repeat(
            x.shape[0], ids_restore.shape[1] + 1 - x.shape[1], 1)  ###mask token是一个共享的可学习的tensor
        x_ = torch.cat([x[:, 1:, :], mask_tokens], dim=1)  ## 先拼接在一起
        x_ = torch.gather(
            x_,
            dim=1,
            index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  ##随后根据ids_restore来进行提取
        x = torch.cat([x[:, :1, :], x_], dim=1) ##与cls——token相加

        # add pos embed
        x = x + self.decoder_pos_embed   ###这里的位置编码还是一个不可以学习的tensor
