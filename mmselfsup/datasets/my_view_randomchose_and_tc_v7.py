...
from .pipelines import montage_PIL,random_choice,transform_center,v11_loc_trans


'''该脚本是对应于my_moco_v11 '''

@DATASETS.register_module()
class RandomchoiceAndTCViewV7(BaseDataset):  ##
    ...
    def __getitem__(self, idx):
        ...
        img_3 = v11_loc_trans(img)

        img_4,order = random_choice(img)

        img_5 = transform_center(img)

        ##
        multi_views = list(map(lambda trans: trans(img), self.trans))
        multi_views.extend([img_2,img_3,img_4,img_5])
        ...
        #     ]
        return dict(img=multi_views,order=order)
